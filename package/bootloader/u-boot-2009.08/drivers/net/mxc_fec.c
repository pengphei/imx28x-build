			      ...
			      int dup_spd)
{
	...
	int ret;
	int delay = 20;	//modified by luozhizhuo for imx280
...
#endif
	} while ((val == 0 || val == 0xffff) && (delay-- > 0)); //modified by luozhizhuo for imx280
#endif
	mydbg("PHY_PHYIDR1=0X%x\n",val);
	__fec_mii_read(fecp, CONFIG_FEC0_PHY_ADDR, PHY_PHYIDR2, &val);
	mydbg("PHY_PHYIDR2=0X%x\n",val);
...
int fec_send(struct eth_device *dev, volatile void *packet, int length)
{
	...
	do {
		...
		if (!(phyStatus & PHY_BMSR_LS)) {
			if (waiting % 1000 == 0) 
				mydbg("FEC: Link is down %x\n", phyStatus);
			continue;
//			return -1;
		} else {
			mydbg("FEC: Link is up %x\n", phyStatus);
			break;
		}
	} while(1);
	mydbg("waiting=%d\n",waiting);
	mydbg("FEC: Link status = 0x%x\n", phyStatus);
	/* section 16.9.23.3
	 * Wait for ready
	 */
	j = 0;
